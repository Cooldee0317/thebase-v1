...
const fetchFarmsPrices = async (farms) => {
  ...
  const BiLLUsdtFarm = farms.find((farm) => farm.pid === wildWethFarmPid);
  const BiLLPriceUsdc =
    BiLLUsdtFarm.tokenPriceVsQuote > 0
      ? new BigNumber(BiLLUsdtFarm.tokenPriceVsQuote).times(wethPriceUsdt)
      : BIG_ZERO;
